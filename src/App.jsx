...
function AlertBanner({ alerts, onDismiss }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", position: "sticky", top: 0, zIndex: 200, background: c.bg, borderBottom: `1px solid ${c.border}` }}>
      <span style={{ fontSize: 16, flexShrink: 0 }}>{alert.icon}</span>
      <p style={{ flex: 1, fontSize: 12, color: c.color, fontWeight: 500 }}>{alert.text}</p>
      {alerts.length > 1 && (
        <span style={{ fontSize: 11, color: c.color, opacity: 0.7, flexShrink: 0 }}>+{alerts.length - 1} more</span>
      )}
      <button onClick={onDismiss} style={{ background: "none", border: "none", color: c.color, cursor: "pointer", fontSize: 14, flexShrink: 0, opacity: 0.7 }}>✕</button>
    </div>
  );
}
...
export default function App() {
  ...
  // ── Unified AI Brain ───────────────────────────────────────────
  const ai = useAI({
    userId, entries, accounts, expCats, incCats, budgets, savingsGoals, 
    onEntriesChange:      u => setEntries(p      => typeof u === "function" ? u(p) : u),
    onBudgetsChange:      u => setBudgets(p      => typeof u === "function" ? u(p) : u),
    onSavingsGoalsChange: u => setSavingsGoals(p => typeof u === "function" ? u(p) : u),
  ...
  const renderTab = () => {
    switch (activeTab) {
      case "expense":
        return (
          <ExpenseTracker
            ...
            onEntriesChange={setEntries} ai={ai}
          />
        );
      case "accounts":
        return (
      ...
      case "budget":
        return (
          <BudgetGoals
            userId={userId} entries={entries} expCats={expCats}
            budgets={budgets} savingsGoals={savingsGoals}
            onBudgetsChange={setBudgets} onSavingsGoalsChange={setSavingsGoals}
            ai={ai}
          />
        );
      case "insights":
    ...
    <div className="app-shell">
      ...
      {showTransfer && (
        <TransferModal
          ...
          userId={userId}
          today={today}
          onTransferDone={newEntries => setEntries(prev => [...newEntries, ...prev])}
      ...
      <main className="main-content">
        {appReady ? (
          <>
            {!alertsDismissed && (
              <AlertBanner alerts={ai.alerts} onDismiss={() => setAlertsDismissed(true)} />
            )}
            {renderTab()}
          </>
        ) : (
