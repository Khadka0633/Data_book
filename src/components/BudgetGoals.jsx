...
[{"category":"name","suggested":number,"avg":number,"reason":"short reason","trend":"stable|increasing|decreasing"}]`,
      ...
      setAiGenerated(true);
    } catch (err) { setAiError(err.message || "Failed to generate suggestions."); }
  ...
  const addBudget = async () => {
    ...
    try {
      ...
      syncBudgets([...budgets, created]);
      setBForm({ category: "", limit: "" }); setBError("");
  ...
  const addSavingsGoal = async () => {
    ...
    try {
      ...
      setSForm(f => ({ ...f, name: "", target: "", current: "" })); setSError("");
    ...
    <div className="page">
      ...
      <div className="card">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <button onClick={generateAiBudgets} disabled={aiLoading || entries.length < 5}
            style={{ background: "var(--accent)", color: "#fff", border: "none", borderRadius: "var(--radius-sm)", padding: "8px 14px", fontSize: 12, fontWeight: 600, whiteSpace: "nowrap", cursor: aiLoading || entries.length < 5 ? "not-allowed" : "pointer", opacity: aiLoading || entries.length < 5 ? 0.6 : 1 }}>
            {aiLoading ? "Analyzing..." : aiGenerated ? "↻ Re-analyze" : "✨ Suggest Budgets"}
          </button>
        </div>

        {!aiGenerated && !aiLoading && (
          <div style={{ textAlign: "center", padding: "20px 16px", background: "var(--surface-2)", borderRadius: "var(--radius-md)", border: "1px dashed var(--border)" }}>
            <p style={{ fontSize: 28, marginBottom: 8 }}>💡</p>
            <p style={{ fontSize: 13, color: "var(--text)", fontWeight: 600, marginBottom: 4 }}>Let AI set your budgets</p>
            <p style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.6 }}>Click "Suggest Budgets" to get AI-recommended limits based on your spending patterns.</p>
          </div>
        )}
        ...
        {aiLoading && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[1,2,3,4].map(i => (
              <div key={i} style={{ height: 72, borderRadius: "var(--radius-md)", background: "var(--surface-2)", border: "1px solid var(--border)", animation: "shimmer 1.5s ease-in-out infinite", animationDelay: `${i * 0.1}s` }} />
            ))}
        ...
        {aiError && (
          <div style={{ padding: "12px 14px", borderRadius: "var(--radius-md)", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", color: "var(--red)", fontSize: 13 }}>⚠️ {aiError}</div>
        )}

        {aiSuggestions.length > 0 && (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <p style={{ fontSize: 12, color: "var(--text-muted)" }}>{aiSuggestions.length} suggestion{aiSuggestions.length !== 1 ? "s" : ""}</p>
              <button onClick={applyAllSuggestions} style={{ background: "rgba(34,197,94,0.12)", color: "var(--green)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: "var(--radius-sm)", padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>✓ Apply All</button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                return (
                  <div key={i} style={{ background: "var(--surface-2)", borderRadius: "var(--radius-md)", padding: "14px 16px", border: "1px solid var(--border)", opacity: alreadySet ? 0.5 : 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                      ...
                      {!alreadySet ? (
                        <button onClick={() => applySuggestion(s)} style={{ background: "var(--accent)", color: "#fff", border: "none", borderRadius: "var(--radius-sm)", padding: "5px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", flexShrink: 0 }}>Set Budget</button>
                      ) : (
      ...
      <div className="card">
        ...
        {availableCats.length > 0 && (
          <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            <select className="input" style={{ flex: 2, minWidth: 140 }} value={bForm.category}
              onChange={e => { setBForm(f => ({ ...f, category: e.target.value })); setBError(""); }}>
                ...
                <div key={b.id} style={{ background: "var(--surface-2)", borderRadius: "var(--radius-md)", padding: "14px 16px", border: over ? "1px solid rgba(239,68,68,0.3)" : near ? "1px solid rgba(249,115,22,0.3)" : "1px solid var(--border)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontSize: 13, color: over ? "var(--red)" : "var(--text-muted)" }}>रु{spent.toLocaleString()} / रु{b.limit.toLocaleString()}</span>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5 }}>
                    <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{over ? `रु${(spent - b.limit).toLocaleString()} over budget` : `रु${(b.limit - spent).toLocaleString()} remaining`}</span>
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      {avg3 && <span style={{ fontSize: 10, color: "var(--text-muted)" }}>3-mo avg: रु{avg3.toLocaleString()}</span>}
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            {accounts.length > 0 && (
              <select className="input" style={{ flex: 1, minWidth: 130 }} value={sForm.accountId}
                onChange={e => setSForm(f => ({ ...f, accountId: e.target.value }))}>
        ...
        {savingsGoals.length === 0 ? (
          <p className="empty-msg">No savings goals yet. Add one above or ask Nexus AI in Insights.</p>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {savingsGoals.map(g => {
                ...
                <div key={g.id} style={{ background: "var(--surface-2)", borderRadius: "var(--radius-md)", padding: "14px 16px", border: done ? "1px solid rgba(34,197,94,0.3)" : "1px solid var(--border)" }}>
                  ...
                  {editGoal?.id === g.id && (
                    <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                      <input className="input" type="number" placeholder="Current amount" value={editGoal.current}
                        onChange={e => setEditGoal(eg => ({ ...eg, current: e.target.value }))} style={{ flex: 1 }} autoFocus />
