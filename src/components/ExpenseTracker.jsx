...
export default function ExpenseTracker({ userId }) {
  ...
  const [form, setForm] = useState({
    type: "expense", amount: "", category: "", note: "", date: today, accountId: "",
  });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setAccounts(accountsRes.map(acc => ({ ...acc, group: acc.group || "cash" })));
  ...
  const accountBalances = useMemo(() => {
    ...
    entries.forEach(e => {
      if (!Object.prototype.hasOwnProperty.call(map, e.accountId)) return;
  ...
  return (
    <div className="page">
      ...
      {showAddAcc && <AddAccountForm onSave={addAccount} onCancel={() => setShowAddAcc(false)} />}

      {/* Add / Edit Transaction — full width, no charts */}
      <div className="card">
