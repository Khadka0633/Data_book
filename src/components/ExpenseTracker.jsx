    ...
    const initChart = () => {
      ...
      chartRef.current = new window.Chart(canvasRef.current, {
        ...
        options: {
          ...
          scales: {
            x: { grid: { display: false }, ticks: { font: { size: 10 }, color: "#888", maxRotation: 45, autoSkip: false }, border: { display: false } },
            y: {
              beginAtZero: true,
              grid: { color: "rgba(128,128,128,0.1)" },
              border: { display: false },
              ticks: { font: { size: 11 }, color: "#888", callback: v => v === 0 ? "0" : `रु${v >= 1000 ? (v / 1000).toFixed(1) + "k" : v}`, maxTicksLimit: 5 },
            },
          },
      ...
      <div className="modal-card" onClick={e => e.stopPropagation()} style={{ maxHeight: "90vh", overflowY: "auto", maxWidth: 520, width: "100%" }}>
        <div className="modal-header">
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 99, background: type === "expense" ? "rgba(239,68,68,0.12)" : "rgba(34,197,94,0.12)", color: type === "expense" ? "var(--red)" : "var(--green)", fontWeight: 600, textTransform: "uppercase" }}>{type}</span>
          </div>
          <button className="modal-close" onClick={onClose}>✕</button>
        </div>
        {catEntries.length === 0 ? (
          <p style={{ color: "var(--text-muted)", fontSize: 13, padding: "16px 0" }}>No transactions found.</p>
        ) : (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 16 }}>
              {[{ label: "Total", value: `रु${total.toLocaleString()}` }, { label: "Count", value: catEntries.length }, { label: "Avg", value: `रु${Math.round(avg).toLocaleString()}` }].map(s => (
                <div key={s.label} style={{ background: "var(--surface-2)", borderRadius: "var(--radius-sm)", padding: "8px 10px", textAlign: "center" }}>
                  <p style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 3 }}>{s.label}</p>
            ...
            <div style={{ position: "relative", width: "100%", height: 200, marginBottom: 16 }}>
              ...
              {!hasData && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 13, color: "var(--text-muted)" }}>No data in last 12 months</span></div>}
            </div>
                  ...
                  <div key={e.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                    <div>
                      ...
                      <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}>
                        ...
                        {acc && <span style={{ marginLeft: 6, padding: "1px 5px", borderRadius: 99, background: acc.color + "22", color: acc.color }}>{acc.icon} {acc.name}</span>}
                      </p>
                    </div>
                    <span style={{ fontSize: 13, fontWeight: 700, color: type === "expense" ? "var(--red)" : "var(--green)" }}>{type === "income" ? "+" : "−"}रु{e.amount.toLocaleString()}</span>
                  </div>
                );
              })}
            </div>
...
function useNoteSuggestions(entries, form) {
  ...
  useEffect(() => {
    ...
    const seen = new Set();
    const matches = entries.filter(e => e.type === form.type && (!form.category || e.category === form.category) && e.note && e.note.toLowerCase().includes(query) && !seen.has(e.note) && seen.add(e.note)).slice(0, 5).map(e => e.note);
...
const CAT_COLORS = ["#6366f1","#22c55e","#ef4444","#f97316","#eab308","#06b6d4","#8b5cf6","#ec4899","#14b8a6","#f43f5e","#84cc16","#0ea5e9","#a855f7","#fb923c","#10b981"];
function getRandomColor(existing = []) {
  const unused = CAT_COLORS.filter(c => !existing.includes(c));
  return (unused.length > 0 ? unused : CAT_COLORS)[Math.floor(Math.random() * (unused.length || CAT_COLORS.length))];
}
...
function CategoryManager({ type, categories, onAdd, onDelete, onClose }) {
  ...
  const handleAdd = () => {
    ...
    if (categories.find(c => c.name.toLowerCase() === trimmed.toLowerCase())) return setError("Already exists.");
    ...
    <div className="modal-overlay" onClick={onClose} style={{ zIndex: 200 }}>
      <div className="modal-card" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input className="input" placeholder="Category name..." value={newName} onChange={e => { setNewName(e.target.value); setError(""); }} onKeyDown={e => e.key === "Enter" && handleAdd()} style={{ flex: 1 }} />
          <button className="btn-primary" onClick={handleAdd} style={{ padding: "10px 16px", whiteSpace: "nowrap", width: "auto" }}>+ Add</button>
        </div>
...
function LoadingScreen() {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "60vh", flexDirection: "column", gap: 12 }}>
      ...
      <span style={{ fontSize: 40, color: "var(--accent)", animation: "pulse 1.5s ease-in-out infinite" }}>⬡</span>
...
export default function ExpenseTracker({ userId, accounts, entries, onEntriesChange, ai }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setExpCats(expCatsRes); setIncCats(incCatsRes);
      setForm(f => ({ ...f, accountId: f.accountId || accounts?.[0]?.id || "", category: f.category || expCatsRes[0]?.name || "" }));
    } catch (err) { console.error("Failed to load categories:", err); hasLoaded.current = false; }
  ...
  const handleAddCat = async (type, cat) => {
    ...
    if (type === "expense") setExpCats(prev => [...prev, created]); else setIncCats(prev => [...prev, created]);
  };
  ...
  const monthlyGrouped = useMemo(() => {
    ...
    const seen = new Set(); const collapsed = [];
    filtered.forEach(e => {
      if (!Boolean(e.isTransfer)) { collapsed.push(e); return; }
      if (seen.has(e.id)) return;
      const pair = filtered.find(p => p.id !== e.id && Boolean(p.isTransfer) && p.amount === e.amount && p.date === e.date && p.type !== e.type);
      if (pair) { seen.add(e.id); seen.add(pair.id); const fromEntry = e.type === "expense" ? e : pair; const toEntry = e.type === "income" ? e : pair; collapsed.push({ ...fromEntry, _transferTo: toEntry, _isPair: true }); }
  ...
  const addEntry = async () => {
    ...
    try {
      if (editEntry) {
        const updated = await pb.collection("entries").update(editEntry, { type: form.type, amount: +form.amount, category: form.category, note: form.note, date: form.date, accountId: form.accountId, userId });
        onEntriesChange(entries.map(e => e.id === editEntry ? updated : e));
      } else {
        const created = await pb.collection("entries").create({ type: form.type, amount: +form.amount, category: form.category, note: form.note, date: form.date, accountId: form.accountId, userId, isTransfer: false });
  ...
  const startEdit = e => { setEditEntry(e.id); setShowForm(true); setForm({ type: e.type, amount: String(e.amount), category: e.category, note: e.note, date: e.date, accountId: e.accountId }); };
    ...
    <div className="page" style={{ padding: "16px", gap: 0 }}>
      ...
      {showForm && (
        <div className="modal-overlay" onClick={closeForm} style={{ zIndex: 100 }}>
          <div className="modal-card" onClick={e => e.stopPropagation()} style={{ maxWidth: 420, width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
            <div className="modal-header">
              <h3 className="modal-title">{editEntry ? "Edit" : "Add Transaction"}</h3>
            ...
            <div className="form-group">
              ...
              <div style={{ position: "relative" }}>
                ...
                {showSuggestions && (
                  <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 50, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", overflow: "hidden", boxShadow: "0 4px 16px rgba(0,0,0,0.3)", marginTop: 4 }}>
                    {suggestions.map((s, i) => (
                      <button key={i} onMouseDown={() => { setForm(f => ({ ...f, note: s })); setShowSuggestions(false); }} style={{ display: "block", width: "100%", textAlign: "left", padding: "9px 14px", fontSize: 13, color: "var(--text)", background: "transparent", border: "none", cursor: "pointer", borderBottom: i < suggestions.length - 1 ? "1px solid var(--border)" : "none" }} onMouseEnter={e => e.currentTarget.style.background = "var(--surface-2)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>{s}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              {editEntry && (
                <button onClick={async () => { await pb.collection("entries").delete(editEntry); onEntriesChange(entries.filter(e => e.id !== editEntry)); closeForm(); }} style={{ width: "100%", padding: "10px", borderRadius: "var(--radius-sm)", background: "rgba(239,68,68,0.08)", color: "var(--red)", border: "1px solid rgba(239,68,68,0.2)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>🗑 Delete</button>
              )}
            </div>
          </div>
        </div>
      ...
      {catModal && <CategoryManager type={catModal} categories={catModal === "expense" ? expCats : incCats} onAdd={cat => handleAddCat(catModal, cat)} onDelete={name => handleDeleteCat(catModal, name)} onClose={() => setCatModal(null)} />}
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 16, background: "var(--surface)", borderRadius: "var(--radius-md)", border: "1px solid var(--border)", overflow: "hidden" }}>
        {[
          ...
          <div key={s.label} style={{ flex: 1, padding: "12px 10px", textAlign: "center", borderRight: i < 2 ? "1px solid var(--border)" : "none" }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 700, color: s.color, fontFamily: "'Syne', sans-serif" }}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      {monthlyGrouped.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {monthlyGrouped.map(({ date, entries: dayEntries }) => {
            ...
            const dayIncome  = dayEntries.filter(e => e.type === "income"  && !Boolean(e.isTransfer) && !e._isPair).reduce((s, e) => s + e.amount, 0);
              ...
              <div key={date} style={{ marginBottom: 4 }}>
                ...
                {dayEntries.map((e, idx) => {
                  ...
                  if (e._isPair) {
                    ...
                    return (
                      <div key={`${e.id}-${idx}`} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: "1px solid var(--border)" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text-muted)" }}>रु{e.amount.toLocaleString()}</span>
                          <button onClick={() => handleDelete(e.id)} className={`del-btn ${confirmId === e.id ? "del-btn-confirm" : ""}`}>{confirmId === e.id ? "?" : "✕"}</button>
                        </div>
                      </div>
                    );
                  }
                  return (
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                        <span style={{ width: 8, height: 8, borderRadius: "50%", background: getCatColor(e.category, e.type), flexShrink: 0 }} />
                        <div style={{ minWidth: 0 }}>
                          ...
                          <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}>
                            ...
                            {acc && <span style={{ marginLeft: 6, padding: "1px 5px", borderRadius: 99, background: acc.color + "22", color: acc.color, fontSize: 10 }}>{acc.icon} {acc.name}</span>}
                          </p>
                        </div>
                      </div>
                      <span style={{ fontSize: 14, fontWeight: 700, color: e.type === "income" ? "var(--green)" : "var(--red)", flexShrink: 0, marginLeft: 8 }}>{e.type === "income" ? "+" : "−"}रु{e.amount.toLocaleString()}</span>
                    </div>
                  );
                })}
              </div>
      ...
      `}</style>
      <button className="fab-add" onClick={() => setShowForm(true)} title="Add transaction">+</button>
    </div>
  );
}
